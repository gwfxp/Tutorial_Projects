package com.youdaigc.report.itext.convertor.helper;
...
//        out.format("  %s: %d, %d, %d;%n", attr, rgb[0], rgb[1], rgb[2]);
        byte[] argb = color.getARGB();
        if (argb == null) {
            return;
        }
//        out.format("  %s: rgba(0x%02x, 0x%02x, 0x%02x, 0x%02x);%n", attr, argb[3], argb[0], argb[1], argb[2]);
//        out.format("  %s: rgba(%d, %d, %d, %d);%n", attr, argb[3], argb[0], argb[1], argb[2]);
    }
}